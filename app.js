...
async function refreshMusicDropdown(selectedUrl = "") {
    ...
    if (!visibleTitleInput) {
        ...
        visibleTitleInput.style.cssText = "width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid var(--border-color); background: var(--body-bg, #0f172a); color: var(--text-main); margin-top: 24px; font-weight: 600; outline: none; box-sizing: border-box; transition: border-color 0.2s;";
    } else {
        visibleTitleInput.type = "text";
    ...
    if (!cardWrapper) {
        ...
        cardWrapper.style.cssText = "padding: 20px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--input-bg); margin-bottom: 16px; box-shadow: 0 4px 12px var(--shadow-color); box-sizing: border-box; width: 100%;";
        
        const cardLabel = document.createElement("div");
        cardLabel.style.cssText = "font-size: 0.9rem; font-weight: 700; color: var(--text-main); margin-bottom: 14px; display: flex; align-items: center; gap: 8px; text-transform: uppercase; letter-spacing: 0.5px;";
        ...
        triggerBtn.style.cssText = "width: 100%; padding: 14px 18px; border-radius: 8px; border: 1px solid var(--border-color); background: var(--body-bg, #0f172a); color: var(--text-main); font-weight: 600; display: flex; align-items: center; justify-content: space-between; cursor: pointer; user-select: none; transition: transform 0.1s, border-color 0.2s; box-sizing: border-box;";
    ...
    if (!popupOverlay) {
        ...
        popupOverlay.style.cssText = "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0, 0, 0, 0.75); backdrop-filter: blur(8px); display: none; align-items: center; justify-content: center; z-index: 999999; box-sizing: border-box; padding: 20px; -webkit-user-select: none; user-select: none;";
        
        const popupCard = document.createElement("div");
        popupCard.style.cssText = "width: 100%; max-width: 350px; background: var(--input-bg, #1e293b); border: 1px solid var(--border-color); border-radius: 20px; box-shadow: 0 20px 50px rgba(0,0,0,0.6); display: flex; flex-direction: column; max-height: 80vh; overflow: hidden; animation: popIn 0.22s cubic-bezier(0.34, 1.56, 0.64, 1);";
        
        const popupHeader = document.createElement("div");
        popupHeader.style.cssText = "padding: 16px 20px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; background: rgba(0,0,0,0.1);";
        ...
        scrollContainer.style.cssText = "padding: 16px; overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: 10px; scrollbar-width: thin;";
                ...
                if (Array.isArray(files)) {
                    files.forEach(file => {
                        if (file.name.endsWith(".mp3")) {
                            ...
                            optionRow.innerHTML = `
                                <div style="display: flex; align-items: center; gap: 10px; overflow: hidden; width: 75%;">
                            ...
                            // 🛠️ FIXED HITBOX: Assigned selection event to the complete row container 
                            optionRow.onclick = () => {
                                popupOverlay.style.display = "none";
                                ...
                                // 🔊 Optional 50% Volume Setup: If you run an actual Audio player globally when selected:
                                // if (window.currentAudioInstance) window.currentAudioInstance.src = file.download_url;
                                // window.currentAudioInstance.volume = 0.5;
                            };
                            ...
                            if (multiActionTrigger) {
                                multiActionTrigger.onclick = (e) => {
                                    e.stopPropagation(); // 🛑 Prevents the parent row's selector from executing when clicking settings
