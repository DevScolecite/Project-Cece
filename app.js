...
function renderTimeline(dataArray) {
    ...
    sortedData.forEach((item, index) => {
        ...
        if (item.isCoreMemory && !shouldBlurCard) {
            statePillsMarkup += `<span style="font-size: 0.68rem; font-weight: 800; text-transform: uppercase; background: rgba(234, 179, 8, 0.1); color: #eab308; padding: 3px 10px; border-radius: 30px; border: 1px solid rgba(234, 179, 8, 0.15); letter-spacing: 0.5px; display: inline-flex; align-items: center; gap: 4px;"><i class="fas fa-star" style="font-size: 0.6/rem;"></i> Core Memory</span>`;
        }
...
function selectFloatingMonth(monthValue, monthDisplayName) {
...
async function testGitHubTokenConnection() {
    ...
    try {
        ...
        if (targetApiResponse.ok) {
            ...
            if (feedbackNode) {
                feedbackNode.innerHTML = `<i class="fas fa-check-circle"></i> Connection Verified: Welcome back @${usernameHandle}! Vault is secure.`;
