...
async function async function improveWithAI() {
    ...
    try {
        ...
        const response = await fetch(apiUrl, {
            ...
            headers: { 
                ...
                "HTTP-Referer": "https://localhost:3000", // Optional OpenRouter ranking parameter
    ...
    { role: "user", content: originalText }
        ...
        if (data.choices && data.choices[0]?.message?.content) {
            ...
            playSFX('aiSuccess');
            console.log("Success: Workspace entry enhanced via Gemma 4 on OpenRouter.");
        } else {
            if (data.error) {
