...
function showAuthGate() {
    ...
    if (isRegistered && hasPin) {
        ...
        updateLockscreenStatus("Enter Master Key", false);
    } else {
...
function handleAccountLogin() {
    ...
    if (userIn.toLowerCase() === CREDENTIALS.username && passIn === CREDENTIALS.password) {
        localStorage.setItem('cece_user_authenticated', 'true');
        playSFX('publish');
        
        document.getElementById('auth-stage-login').classList.add('hidden');
        document.getElementById('auth-stage-pin').classList.remove('hidden');
        updateLockscreenStatus("Create Your 4-Digit PIN", false);
        if (statusText) statusText.innerText = "Account Authenticated";
    } else {
        if (statusText) {
        ...
        playSFX('error');
        const loginContainer = document.getElementById('auth-stage-login');
        loginContainer.style.animation = "shake 0.4s ease";
        setTimeout(() => { loginContainer.style.animation = ""; }, 400);
    }
}
...
async function processPinSubmission() {
    const savedPin = localStorage.getItem('cece_master_pin');
    
    if (!savedPin) {
        localStorage.setItem('cece_master_pin', enteredPin);
        updateLockscreenStatus("PIN Configured Setup Success! ✦", false);
        await unlockApplication();
    } else {
        if (enteredPin === savedPin) {
            updateLockscreenStatus("Welcome back, Lance! ✦", false);
            await unlockApplication();
        } else {
...
function triggerShakeAnimation() {
    ...
    if (container) {
        ...
        container.style.animation = 'shake 0.35s ease';
    }
}
